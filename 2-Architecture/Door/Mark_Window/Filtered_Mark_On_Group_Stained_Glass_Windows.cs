        ...
        public Filtered_Mark_Window()
        {
            try
            {
                ...
                foreach (Element element in all_Elements)
                {
                    ...
                    if (Revit_Document_Mark_Window.Document.GetElement(element.GetTypeId()) != null&& element_Type.get_Parameter(Data_Mark_Window.guid_COD)!=null)
                    {
                ...
                Data_Mark_Window.filtered_Group = Data_Mark_Window.filtered_Group.GroupBy(p => p.Id).Select(g => g.First()).ToList();
